...
def back_prop_s4(img, out_1, w1, z_1, grad_w1, grad_b1,
    ...
    grad_w3 += (2 * d_tanh(z_final) * (out_final - img[1])) @ (np.transpose(out_2))
    # grad_w3 += (2 * d_tanh(z_final) * (out_final - img[1])) @ out_2.transpose

    grad_b3 += (2 * d_tanh(z_final) * (out_final - img[1]))

    grad_out_2 = np.transpose(w3) @ (2 * d_tanh(z_final) * (out_final - img[1]))
    # grad_out_2 = w3.transpose @ (2 * d_tanh(z_final) * (out_final - img[1]))

    grad_w2 += (d_tanh(z_2) * grad_out_2) @ (np.transpose(out_1))
    # grad_w2 += (d_tanh(z_2) * grad_out_2) @ out_1.transpose

    grad_b2 += (d_tanh(z_2) * grad_out_2)

    grad_out_1 = np.transpose(w2) @ (d_tanh(z_2) * grad_out_2)
    # grad_out_1 = w2.transpose @ (d_tanh(z_2) * grad_out_2)

    grad_w1 += (d_tanh(z_1) * grad_out_1) @ (np.transpose(img[0]))
    # grad_w1 += (d_tanh(z_1) * grad_out_1) @ img[0].transpose

    # print(grad_w1)

    grad_b1 += (d_tanh(z_1) * grad_out_1)
...
for i in range(0, len(train_s4)):
    out_1 = tanh(forward(train_s4[i][0], w1, b1))
    out_2 = tanh(forward(out_1, w2, b2))
    out_final = tanh(forward(out_2, w3, b3))
...
print("Step 6-2 Accuracy= ", correct_nums / len(test_set))
